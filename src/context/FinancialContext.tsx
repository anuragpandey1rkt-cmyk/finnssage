...
export function FinancialProvider({ children }: { children: ReactNode }) {
      ...
      if (user) {
        if (user.id === 'demo-user-id') {
          ...
          const mockTxns: Transaction[] = [
            { id: '1', user_id: 'demo-user-id', date: new Date().toISOString(), description: 'Starbucks Coffee', amount: 5.50, type: 'expense', category: 'Food & Dining', source: 'manual', created_at: new Date().toISOString() },
            { id: '2', user_id: 'demo-user-id', date: new Date(Date.now() - 86400000).toISOString(), description: 'Uber Ride', amount: 24.00, type: 'expense', category: 'Transportation', source: 'manual', created_at: new Date().toISOString() },
            { id: '3', user_id: 'demo-user-id', date: new Date(Date.now() - 172800000).toISOString(), description: 'Netflix Subscription', amount: 15.99, type: 'expense', category: 'Entertainment', source: 'manual', created_at: new Date().toISOString() },
            { id: '4', user_id: 'demo-user-id', date: new Date(Date.now() - 259200000).toISOString(), description: 'Salary Deposit', amount: 5000.00, type: 'income', category: 'Income', source: 'manual', created_at: new Date().toISOString() },
            { id: '5', user_id: 'demo-user-id', date: new Date(Date.now() - 345600000).toISOString(), description: 'Whole Foods Market', amount: 145.20, type: 'expense', category: 'Groceries', source: 'manual', created_at: new Date().toISOString() },
            { id: '6', user_id: 'demo-user-id', date: new Date(Date.now() - 432000000).toISOString(), description: 'Amazon Purchase', amount: 67.80, type: 'expense', category: 'Shopping', source: 'manual', created_at: new Date().toISOString() },
          ];
          setTransactions(mockTxns);
          const metrics = calculateMetrics(mockTxns, [], 120000); // Assume 120k income
  ...
  const addTransactions = async (txns: Omit<Transaction, "id" | "user_id" | "created_at">[]) => {
    ...
    if (!user) {
      // Offline mode: Generate mock IDs and update local state
      ...
      // Recalculate metrics
      const metrics = calculateMetrics([...transactions, ...newTxns], investments, financialData.annualIncome);
      setFinancialData(prev => ({
        ...prev,
        ...metrics,
  ...
  const uploadStatement = async (file: File) => {
    ...
    // LOCAL DEMO MODE: Bypass Supabase
    if (user.id === 'demo-user-id') {
      try {
        const parsedTxns = await parseStatement(file);
        if (parsedTxns.length === 0) throw new Error("No transactions found.");

        const newTxns = parsedTxns.map((t, index) => ({
          id: `demo-txn-${Date.now()}-${index}`,
          user_id: user.id,
          statement_id: 'demo-statement',
          amount: t.amount,
          type: t.type,
          category: t.category,
        ...
        setTransactions(prev => [...newTxns, ...prev]); // Add new on top

        // Update metrics
        const currentIncome = financialData.annualIncome; // Keep existing income
        const allTxns = [...newTxns, ...transactions];
        const metrics = calculateMetrics(allTxns, investments, currentIncome);
