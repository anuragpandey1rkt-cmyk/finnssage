...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const signInWithMobileMock = async (phone: string, otp: string) => {
        ...
        if (otp === "1234") {
            // SPECIAL BYPASS FOR DEMO USER to avoid rate limits
            if (phone === "9999999999") {
                const mockUser = {
                    id: 'demo-user-id',
                    aud: 'authenticated',
                ...
                const mockSession = {
                    ...
                    refresh_token: 'mock-refresh-demo',
                    expires_in: 3600,
                    token_type: 'bearer',
                    user: mockUser
                } as unknown as Session;

                // Set state directly, bypassing Supabase
                setUser(mockUser);
                ...
                setProfile({
                    ...
                    created_at: new Date().toISOString(),
                    updated_at: new Date().toISOString(),
                    monthly_expenses: 45000 // Mock expenses
                } as any);
